...
public class PowerHeatFunction implements HeatFunction, Serializable {
	...
	public PowerHeatFunction(double likeHeatPow, double minLikeHeatExp,
			double priorHeatPow, double minPriorHeatExp) {
		this(likeHeatPow, minLikeHeatExp, 1.0, priorHeatPow, minPriorHeatExp,
				1.0);
	}
	
	public PowerHeatFunction(double likeHeatPow, double minLikeHeatExp,
			double maxLikeHeatExp, double priorHeatPow, double minPriorHeatExp,
			double maxPriorHeatExp) {
	...
	public double[] getPriorHeatExponents(int chainCount) {
		...
		if(chainCount > 1) {
			for(int i = 1; i < chainCount; i++) {
				...
				heatExponents[i] = minPriorHeatExp
						+ pow(linearValue, priorHeatPower)
						* (maxPriorHeatExp - minPriorHeatExp);
			}
		}
	...
	public double[] getLikelihoodHeatExponents(int chainCount) {
		...
		if(chainCount > 1) {
			for(int i = 1; i < chainCount; i++) {
				double linearValue = 1.0 - i / ((double) chainCount - 1);
				heatExponents[i] = minLikeHeatExp
						+ pow(linearValue, likeHeatPower)
						* (maxLikeHeatExp - minLikeHeatExp);
			}
		}
