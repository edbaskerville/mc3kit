...
public class Graph extends Observable {
	...
	public Graph addNode(Node node) {
		...
		if(node.name != null && nodeNameMap.containsKey(node.name)) {
			throw new IllegalArgumentException(format(
					"Node named %s already in graph.", node.name));
		}
	...
	public Graph addEdge(Edge edge) {
		...
		if(edge.name != null && edgeNameMap.containsKey(edge.name)) {
			throw new IllegalArgumentException(
					"Edge with this name already in graph.");
		}
		
		// Update order before manipulating other state. If edge introduces a
		// cycle,
		// an exception will be thrown but the graph will be unchanged from its
		// previous state.
		updateOrder(edge);
	...
	public Collection<Node> orderedNodesHeadToTails(
			Collection<? extends Node> tails) {
	...
	private void updateOrder(Edge edge) throws IllegalArgumentException {
		...
		if(logger.isLoggable(Level.FINEST)) {
			logger.finest(format("old order: tail %d, head %d", tailOrder,
					headOrder));
		}
		...
		if(logger.isLoggable(Level.FINEST)) {
			logger.finest(format("new order: tail %d, head %d",
					tail.getOrder(), head.getOrder()));
		}
	}
	
	private Set<Node> findAffectedForwardNodes(Edge edge)
			throws IllegalArgumentException {
		...
		while(!stack.isEmpty()) {
			...
			if(top == edge.head) {
				throw new IllegalArgumentException(
						"Adding edge generates directed cycle in graph.");
			}
		...
		for(Node node : orderNodeMap.values()) {
			for(Edge edge : tailNodeMap.get(node)) {
				...
				if(!visited.contains(head)) {
					getLogger().severe(
							format("node %s: dependency %s not yet visited\n",
									node, head));
					valid = false;
				}
			}
