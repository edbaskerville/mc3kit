...
public final class Math {
	...
	public static boolean shouldAcceptMetropolisHastings(RandomEngine rng,
		...
		if(!Double.isInfinite(newLogPrior) && !Double.isNaN(newLogPrior)
			...
			if(priorHeatExponent == Double.POSITIVE_INFINITY
				...
				double logAcceptanceProbability = logProposalRatio
						+ priorHeatExponent * logPriorRatio
						+ likelihoodHeatExponent * logLikelihoodRatio;
	...
	public static double adjustTuningParameter(double param,
			double measuredRate, double targetRate) {
		if(measuredRate == 0) {
			return param / 2;
		}
		...
		// lambda is updated to be the value on that line where rate =
		// targetRate.
		...
		for(int i = 0; i < x.length - 1; i++) {
			if(x[i + 1] < x[i]) {
				throw new IllegalArgumentException(
						"Integrator assumes x-values are in order smallest to largest.");
			}
