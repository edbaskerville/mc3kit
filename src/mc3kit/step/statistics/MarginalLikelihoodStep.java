...
public class MarginalLikelihoodStep implements Step {
	...
	public MarginalLikelihoodStep(String filename, long burnIn, long thin)
	...
	private synchronized void takeValue(long iteration, Chain chain,
			double logLike, double meanLogLikes, double meanAutoLogLikes) {
		...
		List<MargLikeValue> values = collector.takeValue(iteration, chain
				.getChainId(),
				new MargLikeValue(chain.getLikelihoodHeatExponent(), logLike,
						meanLogLikes, meanAutoLogLikes));
		...
		if(values != null) {
			...
			writer.printf("%d\t%.3f\t%.3f\t%.3f", iteration, margLike,
					autoMargLike, margLikeSingle);
			writer.println();
			writer.flush();
		}
	}
	...
	private class MarginalLikelihoodTask implements Task {
		...
		public void step(Chain[] chains) throws MC3KitException {
			...
			if(iterationCount % thin == 0) {
				...
				takeValue(iterationCount, chains[0], logLike, sumLogLikes
						/ logLikeCount, sumAutoLogLikes / autoLogLikes.size());
			}
		}
	}
	...
	private class MargLikeValue implements Serializable {
		...
		MargLikeValue(double tau, double logLike, double meanLogLikes,
				double meanAutoLogLikes) {
			this.tau = tau;
