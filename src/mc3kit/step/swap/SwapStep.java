...
public class SwapStep implements Step {
	...
	private class Swapper implements Task {
		...
		public void step(Chain[] chains) throws MC3KitException {
			...
			double likelihoodHeatExponent0 = chains[0]
					.getLikelihoodHeatExponent();
			double likelihoodHeatExponent1 = chains[1]
					.getLikelihoodHeatExponent();
			
			double logPriorDiff = model1.getLogPrior() - model0.getLogPrior();
			double logLikelihoodDiff = model1.getLogLikelihood()
					- model0.getLogLikelihood();
			...
			if(likelihoodHeatExponent0 == Double.POSITIVE_INFINITY
				...
				double logSwapProb = (priorHeatExponent0 - priorHeatExponent1)
						* logPriorDiff
						+ (likelihoodHeatExponent0 - likelihoodHeatExponent1)
						* logLikelihoodDiff;
			...
			if(iterationCount % statsEvery == 0) {
				List<SwapStats> statsList = collector.takeValue(iterationCount,
						chainIds[0] / 2, new SwapStats(chainIds,
								acceptanceCount));
				
				Logger logger = chains[0].getMCMC()
						.getLogger("mc3kit.SwapStep");
				if(statsList != null && logger.isLoggable(Level.INFO)) {
					...
					logger.log(
							...
							makeMap("swapParity", swapParity.toString(),
									"swapStats", swapStats));
				}
