    ...
    if(iteration % chain.getMCMC().getThin() == 0) {
    	...
    	try {
      	...
      	table.insert(iteration, stateJson.toString());
    		...
    		throw new MC3KitException(format("Could not write out univariate state (chain %d, iteration %d", chainId, iteration));
    	}
    }
  }
    ...
    for(int i = 0; i < vars.size(); i++) {
      proposers[i] = makeVariableProposer(model, vars.get(i).getName());
	    ...
	    if(chain.getMCMC().shouldRestore()) {
	    	...
	    	ISqlJetCursor c = table.lookup(step.getIndexName(), chain.getIteration());
	    	
	    	JsonParser parser = new JsonParser();
	    	JsonArray stateJson = parser.parse(c.getString("state")).getAsJsonArray();
	    	for(int i = 0; i < proposers.length; i++) {
	    		proposers[i].fromJsonObject(stateJson.get(i).getAsJsonObject());
	    	}
	    }
	    else {
				db.createTable(
						format("CREATE TABLE %s (iteration INTEGER, state TEXT)", step.getTableName())
					);
					db.createIndex(
				...
				throw new MC3KitException(
					format("Couldn't create or load univariate proposal table on chain %d", chain.getChainId()), e
				);
		}
  }
