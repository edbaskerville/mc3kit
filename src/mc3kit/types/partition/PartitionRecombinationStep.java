...
public class PartitionRecombinationStep implements Step {
	...
	private void takeStats(MCMC mcmc, long iterationCount, int[] chainIds,
			Map<String, Long> acceptanceCounts) {
		List<Stats> statsList = collector.takeValue(iterationCount,
				chainIds[0] / 2, new Stats(chainIds, acceptanceCounts));
		
		// If all the stats are in, write them to the log.
		Logger logger = mcmc
				.getLogger("mc3kit.partition.PartitionRecombinationStep");
	...
	private class PartitionRecombiner implements Task {
		...
		public void step(Chain[] chains) throws MC3KitException {
			...
			if(iterationCount % statsEvery == 0) {
				takeStats(chains[0].getMCMC(), iterationCount, chainIds,
						acceptanceCounts);
		...
		private void step(Chain[] chains, String varName)
				throws MC3KitException {
			RandomEngine rng = chains[0].getRng();
			
			Model model0 = chains[0].getModel();
			PartitionVariable var0 = (PartitionVariable) model0
					.getVariable(varName);
			...
			PartitionVariable var1 = (PartitionVariable) model1
					.getVariable(varName);
			...
			int[][] newAssign = generateNewAssignments(rng, unif,
					var0.getGroupCount(), oldAssign0, oldAssign1,
					var0.allowsEmptyGroups());
			...
			double logR = tp0 * newLP0 + tl0 * newLL0 + tp1 * newLP1 + tl1
					* newLL1 - tp0 * oldLP0 - tl0 * oldLL0 - tp1 * oldLP1 - tl1
					* oldLL1;
			...
			if(accepted) {
				...
				if(acceptanceCounts.containsKey(varName)) {
					acceptanceCounts.put(varName,
							acceptanceCounts.get(varName) + 1);
				}
				else {
		...
		private int[][] generateNewAssignments(RandomEngine rng, Uniform unif,
				int groupCount, int[] oldAssign0, int[] oldAssign1,
				boolean allowEmptyGroups) {
			...
			do {
				int nRecomb = 2;
				IterableBitSet recombPtBS = mc3kit.util.Random
						.uniformRandomSubset(unif, n, nRecomb);
				...
				for(int i = 0; i < n; i++) {
					...
					if(recombIndex < nRecomb - 1
							&& j == recombPts[recombIndex + 1]) {
						recombIndex++;
						swapping = !swapping;
					}
				}
		...
		private void assignGroups(PartitionVariable var, int[] assignments)
				throws MC3KitException {
	...
	private class Stats {
		...
		Stats(int[] chainIds, Map<String, Long> acceptanceCounts) {
			...
			for(String key : acceptanceCounts.keySet()) {
				acceptanceRates.put(key, acceptanceCounts.get(key)
						/ (double) statsEvery);
			}
		}
	}
