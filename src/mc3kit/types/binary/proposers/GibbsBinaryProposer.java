...
package mc3kit.types.binary.proposers;
...
public class GibbsBinaryProposer extends VariableProposer {
	...
	public void step(Model model) throws MC3KitException {
		...
		if(logger.isLoggable(Level.FINEST)) {
			...
			logger.finest(format("oldLP, oldLL: %f, %f", oldLogPrior,
					oldLogLike));
		}
		...
		if(logger.isLoggable(Level.FINEST)) {
			logger.finest(format("newLP, newLL: %f, %f", newLogPrior,
					newLogLike));
		}
		
		// For this to be a Gibbs step, the ratio of the probability of
		// acceptance (change the value)
		// to rejection (keep the value what it was) needs to be the ratio of
		...
		double logR = priorHeatExp * (newLogPrior - oldLogPrior) + likeHeatExp
