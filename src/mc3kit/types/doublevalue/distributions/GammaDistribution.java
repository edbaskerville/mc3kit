...
public class GammaDistribution extends DoubleDistribution {
	...
	public GammaDistribution setShape(DoubleValued shapeNode)
			throws MC3KitException {
		shapeEdge = updateEdge(shapeEdge, (ModelNode) shapeNode);
		return this;
	}
	
	public GammaDistribution setRate(DoubleValued rateNode)
			throws MC3KitException {
	...
	public GammaDistribution setScale(DoubleValued scaleNode)
			throws MC3KitException {
	...
	public double getLogP(Variable v) {
		...
		double shape = shapeEdge == null ? this.shape
				: getDoubleValue(shapeEdge);
	...
	public static double getLogPRate(double x, double shape, double rate) {
		...
		return shape * log(rate) - logGamma(shape) + (shape - 1.0) * log(x)
				- rate * x;
	}
	...
	public static double getLogPScale(double x, double shape, double scale) {
		...
		return -shape * log(scale) - logGamma(shape) + (shape - 1.0) * log(x)
				- x / scale;
	}
