...
public class BetaDistribution extends DoubleDistribution {
	...
	public BetaDistribution(Model model, double alpha, double beta)
			throws IllegalArgumentException {
		this(model, null, alpha, beta);
	}
	
	public BetaDistribution(Model model, String name, double alpha, double beta)
			throws IllegalArgumentException {
		super(model, name);
		...
		if(alpha <= 0.0 || beta <= 0.0) {
			throw new IllegalArgumentException(
					"alpha and beta must be larger than zero");
		}
	...
	public BetaDistribution setAlpha(DoubleValued alphaNode)
			throws MC3KitException {
		alphaEdge = updateEdge(alphaEdge, (ModelNode) alphaNode);
		return this;
	}
	
	public BetaDistribution setBeta(DoubleValued betaNode)
			throws MC3KitException {
		betaEdge = updateEdge(betaEdge, (ModelNode) betaNode);
		return this;
	}
	...
	public double getLogP(Variable var) {
		...
		double logP = (alpha - 1.0) * log(x) + (beta - 1.0) * log1p(-x)
				- logBetaAB;
		return logP;
	}
