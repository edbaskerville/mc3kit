...
public class GammaDistributionTest {
	...
	public void severalExpRandom() {
		double[] xs = { 1.312683843659727, 0.670858537743178, 0.194262114674794 };
		
		double[] rates = { 0.3129079407081008, 0.0783979706466198,
				0.7181518440646054 };
		
		double[] logPs = { -1.57259544916731, -2.59855118452025,
				-0.470583946194538 };
		...
		for(int i = 0; i < xs.length; i++) {
			...
			assertEquals(logP, GammaDistribution.getLogPRate(x, 1.0, rate),
					1e-10);
			assertEquals(logP, GammaDistribution.getLogPScale(x, 1.0, scale),
					1e-10);
		}
	}
	...
	public void severalGammaRandom() {
		double[] xs = { 0.165520979559969, 1.744962668356796, 3.419642100591502 };
		
		double[] shapes = { 4.3757828851382348, 0.0159208724896113,
				2.1933361329138279 };
		
		double[] rates = { 3.1516953440264173, 0.1952994993719014,
				0.0416127828806077 };
		
		double[] logPs = { -3.85356377761384, -5.04580351596200,
				-5.74176289694918 };
		...
		for(int i = 0; i < xs.length; i++) {
			...
			assertEquals(logP, GammaDistribution.getLogPRate(x, shape, rate),
					1e-10);
			assertEquals(logP, GammaDistribution.getLogPScale(x, shape, scale),
					1e-10);
		}
	}
}
