...
public class NormalDistribution extends DoubleDistribution {
	...
	public NormalDistribution(Model model, String name, double mean,
			double stdDev) {
		super(model, name);
		this.mean = mean;
		this.stdDev = stdDev;
	...
	public NormalDistribution setMean(DoubleValued meanNode)
	...
	public NormalDistribution setVariance(DoubleValued varNode)
			throws MC3KitException {
	...
	public NormalDistribution setStdDev(DoubleValued stdDevNode)
			throws MC3KitException {
		precEdge = updateEdge(precEdge, null);
	...
	public NormalDistribution setPrecision(DoubleValued precNode)
			throws MC3KitException {
		varEdge = updateEdge(varEdge, null);
		stdDevEdge = updateEdge(stdDevEdge, null);
	...
	public double getLogP(Variable v) {
		...
		if(precEdge != null) {
			...
			assert precEdge == null;
			assert varEdge == null;
			return getLogPStdDev(mean, stdDevEdge == null ? this.stdDev
					: getDoubleValue(stdDevEdge), x);
		}
	}
	...
	public static double getLogPStdDev(double mean, double stdDev, double x) {
		assert stdDev > 0.0;
		double d = x - mean;
		return -(log(stdDev) + 0.5 * LOG_TWO_PI) - d * d
				/ (2.0 * stdDev * stdDev);
	}
