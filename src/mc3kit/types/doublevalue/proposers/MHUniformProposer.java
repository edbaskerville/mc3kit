...
public class MHUniformProposer extends VariableProposer {
	...
	public JsonObject toJsonObject() {
		...
		jsonObj.addProperty("r", proposalRadius);
		return jsonObj;
	}
	...
	public void fromJsonObject(JsonObject jsonObj) {
		...
		proposalRadius = jsonObj.getAsJsonPrimitive("r").getAsDouble();
	}
	...
	public void step(Model model) throws MC3KitException {
		...
		chain.getLogger().finest(
				format("oldLP, oldLL: %f, %f", oldLogPrior, oldLogLikelihood));
		...
		chain.getLogger().finest(
				format("oldVal, newVal = %f, %f", oldValue, newValue));
		...
		chain.getLogger().finest(
				format("newLP, newLL: %f, %f", newLogPrior, newLogLikelihood));
		
		boolean accepted = shouldAcceptMetropolisHastings(rng,
				chain.getPriorHeatExponent(),
				chain.getLikelihoodHeatExponent(), oldLogPrior,
				oldLogLikelihood, newLogPrior, newLogLikelihood,
	...
	public void tune(double targetRate) throws MC3KitException {
		proposalRadius = min(
				adjustTuningParameter(proposalRadius, getAcceptanceRate(),
						targetRate), max - min);
	}
}
