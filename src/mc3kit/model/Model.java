 ...
 * variables, distributions, and (deterministic) functions in a directed acyclic
 * graph.
 * 
...
public class Model implements Observer {
	...
	public void beginConstruction() throws MC3KitException {
		if(state != State.UNINITIALIZED) {
			throw new MC3KitException(
					"beginConstruction called with wrong state");
		}
	...
	public void endConstruction() throws MC3KitException {
		...
		Collection<Node> metaGraphNodes = graph
				.orderedNodesHeadToTails(metaVariables);
	...
	public void endConstruction(Map<String, Object> sample)
		...
		for(Variable var : getUnobservedVariables()) {
			...
			if(!sample.containsKey(varName)) {
				throw new MC3KitException(format(
						"Database doesn't contain variable %s", varName));
			}
	...
	public void recalculate(boolean verify, double tol) throws MC3KitException {
		...
		if(verify) {
			...
			if(logPriorDiff > tol || logLikeDiff > tol) {
				throw new MC3KitException(
						format("Too much error in prior (%f, should be %f, diff %f) or likelihood (%f, should be %f, diff %f)",
								preLogPrior, logPrior, logPriorDiff,
								preLogLike, logLikelihood, logLikeDiff));
			}
		}
	}
	...
	private void propagateChanges(boolean isProposal) throws MC3KitException {
		...
		// Map of nodes to parent edges that have changed, for the sake of
		// efficient updating
		Map<ModelNode, Set<ModelEdge>> visitedEdges = new HashMap<ModelNode, Set<ModelEdge>>();
		
		// Queue of nodes to update in topological order, starting with
		// variables
		...
		while(!updateQueue.isEmpty()) {
			...
			// newEdgeHeads.contains(node)) {
			if(changed || changedValueVars.contains(node)
					|| changedEdgeTails.contains(node)) {
		...
		if(!var.isObserved()) {
			if(var.getName() == null) {
				throw new IllegalArgumentException(
						"Unobserved random variables must have a name.");
			}
	...
	public ModelEdge addEdge(ModelNode tail, ModelNode head)
			throws MC3KitException {
	...
	public Map<String, Object> toDbSample() throws MC3KitException {
		Map<String, Object> samp = new LinkedHashMap<>(
				unobservedVariables.size());
		for(Variable var : unobservedVariables) {
	...
	private enum State {
		UNINITIALIZED, IN_CONSTRUCTION, READY, IN_PROPOSAL, PROPOSAL_COMPLETE, IN_REJECTION
	}
	...
	public void update(Observable obj, Object arg1) {
		if(obj instanceof Variable) {
			...
			assert state == State.IN_CONSTRUCTION || state == State.IN_PROPOSAL
					|| state == State.IN_REJECTION;
