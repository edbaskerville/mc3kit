...
public class PriorLikelihoodOutputStep implements Step {
	...
	private synchronized List<LogPriorLikelihoodValue> takeValue(
			long iteration, int index, double logPrior, double logLikelihood) {
		if(collector == null) {
			collector = new Collector<LogPriorLikelihoodValue>(chainCount);
		}
		return collector.takeValue(iteration, index,
				new LogPriorLikelihoodValue(logPrior, logLikelihood));
	}
	...
	private class PLOutputTask implements Task {
		...
		public void step(Chain[] chains) throws MC3KitException {
			...
			if(iterationCount % thin == 0) {
				...
				List<LogPriorLikelihoodValue> plValues = takeValue(
						iterationCount, chainId, model.getLogPrior(),
						model.getLogLikelihood());
				
				if(plValues != null) {
					for(int i = 0; i < chainCount; i++) {
						writer.printf("%d\t%d\t%.3f\t%.3f\n", iterationCount,
								i, plValues.get(i).logPrior,
