...
public class ExampleModel extends Model {
	...
	public ExampleModel(Chain initialChain, double[] data)
		...
		weight = new DoubleVariable(this, "weight", new UniformDistribution(
				this, 0.0, 1.0));
		...
		for(int i = 0; i < 2; i++) {
			means[i] = new DoubleVariable(this, format("d%d.mean", i),
					meanPrior);
			precs[i] = new DoubleVariable(this, format("d%d.prec", i),
					precPrior);
		}
	...
	private class LikelihoodVariable extends Variable {
		...
		/*
		 * The update method is called whenever a node this variable depends on
		 * has changed; random variables should typically just recalculate their
		 * log-probability and call setLogP here.
		 * 
		 ...
		 * but this can be used to optimize updating: e.g., a maximum function
		 * may get updated very frequently without its value actually changing.
		 * 
		...
		public boolean update() {
			...
			for(double x : data) {
				...
				for(int i = 0; i < 2; i++) {
					...
					logPs[i] = NormalDistribution.getLogPPrecision(mean, prec,
							x);
				}
		...
		/*
		 * If you want to avoid calculating the log-probability again when a
		 * proposal is rejected, override these methods to restore a cached
		 * value of the log-probability.
		 */
		@Override
		 ...
		 * based on which edges changed. By default, the optimized update simply
		 * calls the standard update, which should recalculate from scratch.
		 */
		// @Override
